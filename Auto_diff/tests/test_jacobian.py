import numpy as np
from Auto_diff import AD, Jacobian

def test_function_jacobian():
    x = Jacobian([1, 3, 4])
    fun = np.sin(3*x[0] + 2*x[1] - x[2])
    assert isinstance(fun[0], AD), AssertionError('Not an instance of AD.')
    ...
    jacob = AD.get_derivatives(fun)
    values = AD.get_values(fun)
...
def test_multiple_functions_jacobian():
    ...
    assert isinstance(fun[0][0], AD), AssertionError('Not an instance of AD.')
