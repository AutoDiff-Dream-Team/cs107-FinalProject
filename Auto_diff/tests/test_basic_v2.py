...
def test_basic_addition_v():
    ...
    y = FD(2, 0)
    derivative = x + y
    assert (float(derivative.get_value()) == 5.0) & (float(derivative.get_derivative()) == 1.0), Exception(
...
def test_basic_addition_c():
    x = FD(3, 1)
    derivative = x + 2
    assert (float(derivative.val) == 5.0) & (float(derivative.der) == 1.0), Exception(
        f'test_basic_addition_c() has error.')
...
def test_basic_subtraction_v():
    ...
    y = FD(2, 0)
    derivative = x - y
    assert ((float(derivative.val)) == 1.0) & (float(derivative.der) == 1.0), Exception(
...
def test_basic_subtraction_c():
    ...
    derivative = x - 2
    assert ((float(derivative.val)) == 1.0) & (float(derivative.der) == 1.0), Exception(
        f'test_basic_subtraction_c() has error.')
...
def test_basic_multiplication_v():
    ...
    y = FD(2, 0)
    derivative = x * y
    assert ((float(derivative.val) == 6.0) & (float(derivative.der) == 2.0)), Exception(
        f'test_basic_multiplication_v() has error.')
...
def test_basic_multiplication_c():
    ...
    derivative = x * 2
    assert ((float(derivative.val) == 6.0) & (float(derivative.der) == 2.0)), Exception(
        f'test_basic_multiplication_c() has error.')
...
def test_basic_div_v():
    ...
    y = FD(2, 0)
    derivative = x / y
    assert ((float(derivative.val) == 1.50) & (float(derivative.der) == 0.5)), Exception(
        f'test_basic_div_v() has error.')
...
def test_basic_power_v():
    ...
    y = FD(2, 0)
    derivative = x ** y
    assert ((float(derivative.val) == 9.0) & (float(derivative.der) == 6.0)), Exception(
        f'test_basic_power_v() has error.')
...
def test_basic_power_c():
    ...
    derivative = x ** 2
    assert ((float(derivative.val) == 9.0) & (float(derivative.der) == 6.0)), Exception(
        f'test_basic_power_c() has error.')


def test_basic_exponential_v():
    x = FD(3, 1)
    derivative = np.exp(x)
    assert ((float(derivative.val) == np.exp(3)) & (float(derivative.der) == np.exp(3))), Exception(
        f'test_basic_exponential_v() has error.')
...
def test_basic_rsub_c():
    ...
    derivative = 2 - x
    assert (float(derivative.val) == -1.0) & (float(derivative.der) == -1.0), Exception(
        f'test_basic_rsub_c() has error.')


def test_basic_rdiv_c():  # Account for when the divisor is 0
    x = FD(3, 1)
    derivative = 2 / x
    assert (float(derivative.val) == (2 / 3)) & (float(derivative.der) == -(2 / 9)), Exception(
...
def test_basic_rpow_c():
    x = FD(3, 1)
    derivative = 2 ** x
    assert (float(derivative.val) == 8.0) & (float(derivative.der) == 8 * np.log(2)), Exception(
        f'test_basic_rpow_c() has error.')


def test_basic_neg_v():
    x = FD(3, 1)
    derivative = -x
    assert (float(derivative.val) == -3.0) & (float(derivative.der) == -1.0), Exception(
        f'test_basic_neg_v() has error.')
